...
```{r}
h <- heights %>% ggplot(aes(x = height)) + geom_histogram()
...
```{r}
heights %>% ggplot(aes(x = height, fill = sex)) + geom_boxplot(alpha = 1/2) 
```
...
```{r}
heights_mean <- mean(heights$height)
heights_median <- median(heights$height)
